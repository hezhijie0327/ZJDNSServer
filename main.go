...
const (
	...
	MaxCNAMEChain        = 16   // 统一的CNAME链长度和递归深度限制
...
func NewRedisDNSCache(config *ServerConfig, server *RecursiveDNSServer) (*RedisDNSCache, error) {
	rdb := redis.NewClient(&redis.Options{
		...
		PoolSize:     50,        // 硬编码默认值
		MinIdleConns: 10,        // 硬编码默认值
		MaxRetries:   3,         // 硬编码默认值
		PoolTimeout:  5 * time.Second, // 硬编码默认值
...
func (rc *RedisDNSCache) Get(key string) (*CacheEntry, bool, bool) {
	...
	// 更新访问信息（使用硬编码的100ms节流）
	now := time.Now().Unix()
	if now-entry.AccessTime > 0 { // 简化的访问时间更新逻辑
...
func NewConnectionPool(poolSize int, timeout time.Duration) *ConnectionPool {
	// 简化连接池，使用动态池
	maxSize := poolSize * 2
	minSize := poolSize / 2
	if minSize < 10 {
